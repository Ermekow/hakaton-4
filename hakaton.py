...
# cursor.execute ("SELECT * FROM kivano INNER JOIN sulpak ON kivano.price = sulpak.price;)
# ######################### 2. Узнайте самую последнюю модель Iphone в магазинах. ########################
# SELECT product_name, product_name FROM sulpak WHERE product_name in (SELECT product_name FROM kivano WHERE product_name = 'Iphone 12');
...
# SELECT * FROM sulpak  WHERE product_name LIKE 'Lenovo%' or product_name like 'MSI%' or product_name like 'Acer%' or product_name like 'ASUS%' or product_name like 'MacBook%';
...
# select * from sulpak, produsers order by prodused_date;
...
# UPDATE produsers SET produser_company = 'Apple', produser_country = 'kyrgyzstan' WHERE produser_id = (SELECT MAX(produser_id) FROM produsers)
...
# SELECT kivano.product_name,kivano.price FROM kivano LEFT JOIN sulpak ON kivano = sulpak AND kivano != sulpak;
# # 12. Найдите самый последний японский товар который был добавлен в produsers.
# SELECT produser_company FROM produsers WHERE produser_id = (SELECT MAX(produser_id) FROM produsers WHERE produser_country = 'Japan');
...
# cursor.execute("UPDATE produsers SET prodused_date = 2000-00-00 WHERE prodused_date > 1998;")
...
# UPDATE produsers SET produser_country = 'Germany' WHERE produser_country = 'Brazil' AND produser_company = 'Aser';
...
# UPDATE produsers SET produser_company = 'Microsoft' WHERE produser_company = 'Nokia' AND produser_country = 'USA';
